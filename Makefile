...
RST_API_TARGETS := $(DOCS_DIR)/exponential_column_source.rst $(DOCS_DIR)/exponential_volume_source.rst $(DOCS_DIR)/line.rst $(DOCS_DIR)/plane.rst $(DOCS_DIR)/point.rst
...
help:
	...
	@echo "  speed          - Run grheat benchmark placeholders"
	@echo "  readme_images  - Placeholder for README image generation"
...
.PHONY: readme_images readme
readme_images readme:
	@echo "No README image generator is configured for $(PACKAGE)."
...
.PHONY: ruff-check
ruff-check:
	@$(RUN) ruff check grheat tests .github docs/conf.py

.PHONY: black-check
black-check:
	$(RUN) black --check grheat tests .github docs/conf.py
...
.PHONY: speed
speed:
	@echo "No dedicated speed benchmarks are configured for $(PACKAGE)."
...
lite-clean:
	...
	@$(RMR) "$(OUT_ROOT)"
	@$(RMR) "$(DOIT_DB)"
	@$(RMR) .cache dist $(PACKAGE).egg-info build
